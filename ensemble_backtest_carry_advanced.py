...
plt.figure()
plt.plot(data_test["date"], data_test["strategy_usd"], label="USD/UAH Carry Return")
plt.plot(data_test["date"], data_test["strategy_eur"], label="EUR/UAH Carry Return")
plt.legend()
plt.title("Carry Trade Strategy Backtest")
plt.xlabel("Date")
plt.ylabel("Portfolio Value")
plt.grid(True)
plt.tight_layout()
...
for days in forecast_horizons:
    ...
    pred_usd_ret = ensemble_usd.predict(latest_data[features])[0] * days
    pred_eur_ret = ensemble_eur.predict(latest_data[features])[0] * days
